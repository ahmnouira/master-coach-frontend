...
export class RdvReservationComponent implements OnInit {
  ...
  constructor(private router : Router, private location: Location, public window: Window, private rdvService : RdvService) {
  ...
  ngOnInit(): void {
    ...
    this.workingHours = moment.duration(moment(this.heureFin).diff(moment(this.heureDeb))).asHours();
  ...
  isAvailable(num: number): boolean{
    ...
    else  return this.weekDays.filter(day => day.number +1 == num)[0]?.isSelected;
  }

  onDateClick(day) {
    this.selectedAppointments[this.selectedAppointmentIndex].selectedDate = moment(day.value+'/'+ this.viewDate.format('MM/YYYY'), 'DD/MM/YYYY');
    this.selectedDate = moment(day.value+'/'+ this.viewDate.format('MM/YYYY'), 'DD/MM/YYYY');
  }

  onTimeClick(time, appointmentIndex) {
    this.selectedAppointments[appointmentIndex].selectedTimes.forEach(elem => {
  ...
  isCrenauAvailable(date,cren) {
    ...
    console.log(cren)
    let apps = this.coachAppointments.filter(elem => elem.date.includes(dateFormatted) && elem.time.includes(cren.creneauText))
