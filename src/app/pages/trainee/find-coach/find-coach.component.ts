...
export class FindCoachComponent implements OnInit {
  ...
  getUserProfileList() {
    this.userService.getAllUser()
      .subscribe(response => {
        ...
        this.filteredData = (response as any).filter(elem => elem.role =='Coach');
  ...
  getCompétences() {
    this.adminService.getAllCompetances()
      .subscribe(response => {
  ...
  getAccreditations() {
    this.adminService.getAllAccreditations()
      .subscribe(response => {
  ...
  getAllCategorys() {
    this.adminService.getAllCategorys()
      .subscribe(response => {
  ...
  filterInputChanged(event) {
    this.filteredData = this.data.filter(elem =>
      elem.competances?.toLowerCase().includes(event.target.value.toLowerCase())
      || elem.accreditations?.toLowerCase().includes(event.target.value.toLowerCase())
      || elem.categories?.toLowerCase().includes(event.target.value.toLowerCase())
  ...
  goToCoachDEtails(coach) {
    this.router.navigateByUrl('/pages/client/coach-detail', { state: { id: coach } });
  }
}
