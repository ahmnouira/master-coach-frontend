...
export class RdvPayComponent implements OnInit {
  ...
  constructor(private rdvService : RdvService,
              ...
              private tokenStorageService: TokenStorageService, public window: Window) { }
  ...
  ngOnInit(): void {
    ...
    this.selectedAppointments = JSON.parse( history.state?.id.selectedAppointments);
    this.selectedAppointments.forEach(
      elem => {
        elem.moment = moment(elem.selectedDate);
        elem.selectedTimes.forEach(
          time => {
    ...
    this.selectedDate.selectedTime.endTime = (Number(this.selectedDate.selectedTime.creneauText.split(':')[0]) + 1) + ':' + this.selectedDate.selectedTime.creneauText.split(':')[1];
    this.selectedDate.moment = moment(this.selectedDate.selectedDate, 'dddd, DD/MM/YYYY');
  }

  ngAfterViewInit() {
    this.modal.getModal('modalsucces').onClose.subscribe((modal: NgxSmartModalComponent) => {
      // modal is the dismissed modal, so do what you want with it
      this.router.navigate(['/pages/client/rdv/list']);
    });
    this.modal.getModal('modalsucces').onDismiss.subscribe((modal: NgxSmartModalComponent) => {
      // modal is the dismissed modal, so do what you want with it
      this.router.navigate(['/pages/client/rdv/list']);
    });
  }
