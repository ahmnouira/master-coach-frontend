...
export class AnswerQuizComponent implements OnInit {
  ...
  constructor(private router :Router, private coachService : CoachService, private location : Location, private tokenStorageService : TokenStorageService) { }
  ...
  ngOnInit(): void {
    ...
    let answers = this.quizData?.client_answers.filter(elem => elem.client_id == this.tokenStorageService.getUser()._id)
    if(answers == 0 ) {
