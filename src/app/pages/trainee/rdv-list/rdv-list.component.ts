...
export class RdvListComponent implements OnInit {
  ...
  constructor(private router : Router, private location: Location, private rdvService : RdvService, private tokenStorageService : TokenStorageService) {
  ...
  makeGrid(){
    ...
    const daysInMonth = Array.from({length: this.viewDate.daysInMonth()}, (x, i) => this.viewDate.startOf('month').add(i, 'days'));
  ...
  onDateClick(day) {
    console.log(moment(day.value+'/'+ this.viewDate.format('MM/YYYY'), 'DD/MM/YYYY').format('DD/MM/YYYY'))
    this.selectedDate = moment(day.value+'/'+ this.viewDate.format('MM/YYYY'), 'DD/MM/YYYY');
    this.sessionList = this.data.filter(elem =>
      elem?.date?.toLowerCase().includes(moment(day.value+'/'+ this.viewDate.format('MM/YYYY'), 'DD/MM/YYYY').format('YYYY-MM-DD'))
    );
  }
  ...
  onActionClicked(element: any) {
    ...
    if (element.action == 'view') {
      this.router.navigateByUrl('/pages/client/rdv/detail', { state: { id: element.item } });
    }
