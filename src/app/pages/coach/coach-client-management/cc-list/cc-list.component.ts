...
export class CcListComponent implements OnInit {
  ...
  filterInputChanged(event) {
    if(this.filterString ==''){
      this.coachUsers = this.coachObject.clients.length > 0 ? [...this.coachObject.clients]: [];
    }
    else {
      this.coachUsers = this.coachObject.clients.filter(elem =>
  ...
  deleteUser(userElement){
    ...
    this.userService.updateUser(this.coachObject, this.coachObject._id).subscribe(
      data => {},
  ...
  async saveUser() {
    ...
    if(userTeam && userTeam?._id != this.selectedTeam) { // Check user is in team and user's team is different than
    ...
    await this.userService.updateUser(this.selectedUserToupdate, this.selectedUserToupdate._id).subscribe(res => {
  ...
  addUserToTeam(user, teamId){
    ...
    this.coachService.UpdateTeam(team).subscribe(addData => {
      console.log(addData)
    }, error => {console.log(error)})
  }
