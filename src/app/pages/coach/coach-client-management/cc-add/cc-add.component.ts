...
export class CcAddComponent implements OnInit {
  ...
  saveUser() {
    ...
    this.form['username'] = this.form['nom'].toLowerCase()+'.'+this.form['prenom'].toLowerCase();
    this.form.photo = this.form.photo ? this.form.photo : '/assets/img/common/utilisateur.png';
    this.userService.saveUser(this.form).subscribe(newUser => {
      console.log(newUser);
      let a =[];
      a.push(newUser.user);
      if(this.teamsList.length > 0 && this.selectedTeam != 'NO_TEAM') {
        console.log(this.selectedTeam)
        let team = this.teamsList.filter(elem => elem._id == this.selectedTeam)[0]
  ...
  getTeamsList() {
    this.coachService.GetTeams(this.tokenStorageService.getUser()._id).subscribe(data => {
      console.log(data)
