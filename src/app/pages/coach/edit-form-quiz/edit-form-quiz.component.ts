...
export class EditFormQuizComponent implements OnInit {
  ...
  addResponse(indexQuestion){
    this.quizData.questions[indexQuestion].questionResponses.push({
      responsePosition : this.quizData.questions[indexQuestion].questionResponses.length,
      responseText: ''
    })
  }
  removeResponse(indexQuestion, indexREsp) {
    this.quizData.questions[indexQuestion].questionResponses.splice(indexREsp,1);
  }

  questionChoixUniqueChanged(indexQuestion){
    this.quizData.questions[indexQuestion].questionChoixMultiple = !this.quizData.questions[indexQuestion].questionChoixUnique;
    this.quizData.questions[indexQuestion].questionType = this.quizData.questions[indexQuestion].questionChoixUnique? 'unique':'multiple';
  }

  questionChoixMultipleChanged(indexQuestion){
    this.quizData.questions[indexQuestion].questionChoixUnique = !this.quizData.questions[indexQuestion].questionChoixMultiple;
    this.quizData.questions[indexQuestion].questionType = this.quizData.questions[indexQuestion].questionChoixUnique? 'unique':'multiple';
  }
