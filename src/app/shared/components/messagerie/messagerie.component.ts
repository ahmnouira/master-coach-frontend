...
export class MessagerieComponent implements OnInit, AfterViewInit {
        ...
        await cnv.getParticipants().then(users => {
          users.map((user:any) => {
            if (user.identity != this.currentUser.email) {
              let participant1Object = this.allusersObjects.filter(elem => elem.email == user.identity)[0];
              cnv.channelState.friendlyName = participant1Object?.prenom + ' ' + participant1Object?.nom?.toUpperCase();
  ...
  getConversationName(conversation) {
    if(conversation.channelState.friendlyName.includes('AND')) {
      ...
      let participant1Object = this.allusersObjects.filter(elem => elem.email == participant1)[0];
      let participant2Object = this.allusersObjects.filter(elem => elem.email == participant2)[0];
      if (participant1 != this.currentUser.email) {
        conversation.channelState.friendlyName = participant1Object?.prenom + ' ' + participant1Object?.nom?.toUpperCase();
        conversation.channelState.userImage = participant1Object?.photo;
        return conversation.channelState.friendlyName;
      } else {
        conversation.channelState.friendlyName = participant2Object?.prenom + ' ' + participant2Object?.nom?.toUpperCase();
  ...
  getUserName(userEmail) : string {
    let participant1Object = this.allusersObjects.filter(elem => elem.email == userEmail)[0];
    return participant1Object?.prenom + ' ' + participant1Object?.nom?.toUpperCase();
  }
  ...
  deleteConversation(conv) {
    this.client.getConversationBySid(conv.sid).then(conv => {
      conv.delete().then(deleted => {
        console.log('deleted from twilio');
        this.conversationService.deleteConversation(conv.sid).subscribe(
          data => {
  ...
  importFile(event:any) {
    ...
    if(event.target.files && event.target.files.length) {
      ...
      if (!allowed_types.includes(event.target.files[0].type)) {
        this.uploadError = 'Seulement ces extensions de fichiers sont acceptées ( JPG | PNG | DOCX | DOC | XLSX | XLS | PPTX | PPT | PDF  )';
        return false;
      }
      ...
      file.arrayBuffer().then(buff => {
        ...
        this.msgContent = this.fileToUploadData.fileName + '; ' + this.msgContent;
        return true;
      })
