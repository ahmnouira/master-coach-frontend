...
export class VisioConferenceComponent implements OnInit, AfterViewInit {
  ...
  getToken () {
    let token = this.conversationService.getVideoRoomToken({room:this.roomName}).subscribe(
      data =>{
  ...
  disableVideo  () {
    ...
    const videoTrack = [...this.room.localParticipant.videoTracks.values()][0].track;
    if (this.camStatus) {
      videoTrack.restart();
    }else{
      const videoTrack = [...this.room.localParticipant.videoTracks.values()][0].track;
  ...
  disableMicro () {
    ...
    const audioTrack = [...this.room.localParticipant.audioTracks.values()][0].track;
    if (this.microStatus) {
      audioTrack.enable();
    }else{
      const audioTrack = [...this.room.localParticipant.audioTracks.values()][0].track;
