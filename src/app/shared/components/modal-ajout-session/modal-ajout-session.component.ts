...
export class ModalAjoutSessionComponent implements OnInit {
  ...
  constructor(public ngxSmartModalService: NgxSmartModalService, private rdvService : RdvService, private localeService : BsLocaleService, private tokenStorageService : TokenStorageService) {
  ...
  ngAfterViewInit() {
    this.ngxSmartModalService.getModal('modalajoutsession').onOpen.subscribe((modal) => {
  ...
  saveSession() {
    ...
    let param = {
      data: {
        date: moment(this.selectedDate).tz(this.selectedTimezone).format('YYYY-MM-DD'),
        time: moment(this.heureDeb).format('HH:mm:ss'),
        endTime: moment(this.heureFin).format('HH:mm:ss'),
        users: this.isIndividual ? [this.selectedUserId] : [...this.teams.filter(elem => elem._id == this.selectedTeamId)[0].users],
  ...
  selectedDateChanged() {
    this.heureDeb.setFullYear(this.selectedDate.getYear(),this.selectedDate.getMonth(),this.selectedDate.getDate());
    this.heureFin.setFullYear(this.selectedDate.getYear(),this.selectedDate.getMonth(),this.selectedDate.getDate());
  }
}
