...
export class ModalGoogleAgendaComponent implements OnInit {
  ...
  constructor(public ngxSmartModalService: NgxSmartModalService, public googleService : GoogleApiWrapperService) {
  }
  ...
  ngAfterViewInit() {
    this.ngxSmartModalService.getModal('modalgoogleagenda').onOpen.subscribe((modal) => {
      ...
      if(this.isLoggedIn) {
        ...
        this.delay(10000).then(delay => {this.ngxSmartModalService.closeLatestModal()});
      }
    });
  }
